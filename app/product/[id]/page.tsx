...
export default function ProductPage({ params }: { params: { id: string } }) {
  ...
  const speak = (textArr: string[]) => {
    ...
    try {
      ...
      const langRate = lang === 'pidgin' ? 0.7 : 0.85
      setTimeout(() => {
        let i = 0
        const next = () => {
  ...
  const getTexts = (oid: string) => {
    ...
    return {
      title: isMuted ? "Order Page" : "Order Page - Click YES",
      desc: meta.name + " - Total " + total.toLocaleString() + " naira",
      confirm: ["Order confirmed.", "Order ID " + oid + ".", "Total " + total.toLocaleString() + " naira.", "Please click Pay with MTN MoMo."],
      success: ["Payment secured.", "Payment of " + total.toLocaleString() + " naira secured in escrow.", "AfricanIES will deliver."],
      insufficient: ["Insufficient MoMo balance.", "Required " + total.toLocaleString() + " naira.", "Balance " + balance.toLocaleString() + " naira."]
    }
  }
          ...
          {step==='confirmed' && (
            <div className="mt-3 p-3 bg-blue-50 border-2 border-blue-300 rounded-xl">
              ...
              <div className="flex gap-2 mt-2">
                {isMuted ? <button disabled className="flex-1 py-2 bg-gray-200 text-gray-500 rounded-full text-xs font-bold">Voice coming soon</button> : <button onClick={()=>speak(ot.confirm)} className="flex-1 py-2 bg-white border rounded-full text-xs font-bold">Replay</button>}
          ...
          {step==='success' && (
            <div className="mt-3 p-3 bg-green-50 border-2 border-green-400 rounded-xl">
              ...
              <div className="flex gap-2 mt-3">
                {isMuted ? <button disabled className="flex-1 py-2 bg-gray-200 text-gray-500 rounded-full text-xs font-bold">Voice coming soon</button> : <button onClick={()=>speak(ot.success)} className="flex-1 py-2 bg-black text-white rounded-full text-xs font-bold">Replay Full</button>}
